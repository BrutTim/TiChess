        ...
        case "gameState" | "move" =>
          event.fen match
            case Some(fen) =>
              Fen.parse(fen) match
                case Right(game) =>
                  val state = AppState(game, startGame = game, moveHistory = parseMoves(event.moves.getOrElse("")))
                  ...
                  checkTurnAndPlay(gameId, state, botColor, timeMs, thinking, () => currentState.get())
                case Left(err) =>
